...
//   plus:      fri_state/session_state, connection_quality, safety_state, command_mode,
...
TEST_F(KukaFRIHardwareInterfaceExportTest, ExportsExpectedStateInterfaces)
{
  ...
  {
    expected_names.push_back(std::string(hardware_interface::FRI_STATE_PREFIX) + "/" + name);
  }
...
//   runtime_config/control_mode, runtime_config/receive_multiplier, runtime_config/send_period_ms
...
TEST_F(KukaFRIHardwareInterfaceExportTest, ExportsExpectedCommandInterfaces)
{
  ...
  std::vector<std::string> expected_names;
  for (const char * name :
       {hardware_interface::CONTROL_MODE, hardware_interface::RECEIVE_MULTIPLIER,
        hardware_interface::SEND_PERIOD})
  {
    expected_names.push_back(std::string(hardware_interface::CONFIG_PREFIX) + "/" + name);
  }

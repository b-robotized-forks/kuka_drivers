...
// Interfaces exported by the removed raw-pointer export_state_interfaces()/
// export_command_interfaces() common to all three RSI hardware interface variants:
//   per joint: <joint>/position (state and command)
//   plus:      gpio/<gpio state name> (state), gpio/<gpio command name> (command)
...
std::vector<std::string> expected_base_state_names()
{
  ...
  for (size_t i = 1; i <= kJointCount; ++i)
  {
    names.push_back("joint_" + std::to_string(i) + "/" + hardware_interface::HW_IF_POSITION);
  }
...
std::vector<std::string> expected_extended_state_names()
{
  ...
  {
    names.push_back(std::string(hardware_interface::STATE_PREFIX) + "/" + name);
  }
  return names;
}
...
std::unique_ptr<hardware_interface::System> initialize(const rclcpp::Logger & logger)
{
  ...
  auto hw =
    std::make_unique<hardware_interface::System>(std::make_unique<HardwareInterfaceT>());
